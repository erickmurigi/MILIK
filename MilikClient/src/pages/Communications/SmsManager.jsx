...
const ComposePanel = ({ open, onClose, businessId, tenants, onSent }) => {
  ...
  const filteredTenants = useMemo(() => {
    ...
    return t ? tenants.filter(tn => (tn.name || "").toLowerCase().includes(t) || (tn.phone || "").includes(t)) : tenants;
...
const SmsManager = () => {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await adminRequests.get(`/communications/sms-logs?business=${businessId}&limit=500&channel=sms`);
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(l => [l.recipient, l.to, l.recipientName, l.body, l.message, l.type, l.templateName]
                            ...
                            <div className="border border-slate-200 bg-white p-4">
                              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 mb-3">
                                {[
                                  ...
                                  { label: "Phone", value: log.recipient || log.to || "—" },
