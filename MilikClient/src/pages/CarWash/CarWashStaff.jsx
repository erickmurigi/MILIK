...
const CarWashStaff = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const payload = await carWashApi.listStaff({
        limit: PAGE_SIZE,
        page,
        search: appliedFilters.search || undefined,
        active: appliedFilters.status === "active" ? true : appliedFilters.status === "inactive" ? false : undefined,
      });
      const staff = normalizeListPayload(payload, "staff");
      setRows(staff);
      setPagination(payload?.pagination || { page, limit: PAGE_SIZE, total: staff.length, pages: 1 });
      setExpandedIds([]);
    } finally {
      setLoading(false);
    }
  ...
  const openEdit = (row) => {
    ...
    setForm({ name: row.name || "", phone: row.phone || "", role: row.role || "", active: row.active !== false });
    setShowModal(true);
  };
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || "Unable to save staff");
    }
  };
                  ...
                  <React.Fragment key={row._id}>
                    ...
                    {expanded && (
                      <tr className="border-b border-slate-200 bg-[#F8FBF9]">
                        <td colSpan={6} className="px-10 py-2 text-[11px] text-slate-600">
                          <div className="grid gap-3 md:grid-cols-4">
                            ...
                            <div><span className="font-extrabold uppercase text-slate-500">Staff ID:</span> {row._id}</div>
                          </div>
                        </td>
                      </tr>
                    )}
        ...
        <Modal
          ...
          footer={
            <>
              ...
              <button type="submit" form="carwash-staff-form" className="bg-[#0B3B2E] px-4 py-2 text-xs font-bold text-white hover:bg-[#0A3127]">
                Save Staff
              </button>
            </>
          }
        >
          <form id="carwash-staff-form" onSubmit={submit} className="grid gap-3 md:grid-cols-2">
            ...
            <div>
              ...
              <input className={inputClass} value={form.role} onChange={(event) => setForm((prev) => ({ ...prev, role: event.target.value }))} />
            </div>
