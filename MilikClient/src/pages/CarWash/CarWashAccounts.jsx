...
import {
  ...
  FaTimes, FaUser, FaExclamationTriangle,
} from "react-icons/fa";
...
const CarWashAccounts = () => {
  ...
  const stats = useMemo(() => {
    ...
    const totalOwed = accounts.reduce((s, a) => s + Math.max(0, Number(a.currentBalance || 0)), 0);
    const overLimit = accounts.filter((a) => a.creditLimit > 0 && a.currentBalance > a.creditLimit).length;
    return { total, totalOwed, overLimit };
  }, [accounts]);
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        <div className="flex-shrink-0 border-b border-slate-200 bg-white px-3 py-2">
          ...
          <div className="mt-2 flex flex-wrap gap-2 text-[11px]">
            ...
            <span className="rounded border border-red-200 bg-red-50 px-2 py-0.5 font-bold text-red-700">Total owed: {fmt(stats.totalOwed)}</span>
            {stats.overLimit > 0 && (
                ...
                return (
                  <React.Fragment key={acc._id}>
                    <tr className={`border-b border-slate-100 ${overLimit ? "bg-amber-50/60" : "bg-white hover:bg-slate-50"} cursor-pointer`} onClick={() => toggleExpand(acc)}>
                      ...
                      <td className="px-3 py-2">
                        <span className={`inline-flex rounded px-2 py-0.5 text-[10px] font-bold uppercase ${acc.accountType === "monthly" ? "bg-violet-100 text-violet-700" : "bg-blue-100 text-blue-700"}`}>
                          {acc.accountType}
                        </span>
                      ...
                      <td className="px-3 py-2 text-right">
                        <div className="inline-flex gap-1" onClick={(e) => e.stopPropagation()}>
                          ...
                          <button onClick={() => { toggleExpand(acc); if (!expanded) loadStatements(acc._id); }} className="inline-flex items-center gap-1 border border-slate-200 bg-white px-2 py-0.5 text-[10px] font-bold text-slate-600 hover:bg-slate-50">
                            {expanded ? "Hide" : "View"}
                          </button>
                        </div>
                      </td>
